-- Update reward_catalog: set real Pokémon names + generation column
-- Matches any image_url ending in /<number>.png (PokeAPI official-artwork pattern)
-- Run in: Supabase Dashboard → SQL Editor → New Query → Run

-- Step 1: add generation column if it doesn't exist yet
ALTER TABLE reward_catalog
  ADD COLUMN IF NOT EXISTS generation smallint;


WITH pokemon_names(dex, pokemon_name) AS (
  VALUES
  -- Gen 1
  ...
  (433,'Chingling'),(434,'Stunky'),(435,'Skuntank'),
  ...
  (1017,'Ogerpon'),
  (1018,'Gouging Fire'),(1019,'Raging Bolt'),
  (1020,'Iron Boulder'),(1021,'Iron Crown'),
  (1022,'Terapagos'),
  (1023,'Pecharunt'),
  (1024,'Alolan Raichu'),(1025,'Eternamax Eternatus')
)
-- Step 2: update name + generation in one pass
UPDATE reward_catalog
SET
  name       = pn.pokemon_name,
  generation = CASE
    WHEN pn.dex <=  151 THEN 1
    WHEN pn.dex <=  251 THEN 2
    WHEN pn.dex <=  386 THEN 3
    WHEN pn.dex <=  493 THEN 4
    WHEN pn.dex <=  649 THEN 5
    WHEN pn.dex <=  721 THEN 6
    WHEN pn.dex <=  809 THEN 7
    WHEN pn.dex <=  905 THEN 8
    ELSE 9
  END
FROM pokemon_names pn
WHERE (regexp_match(reward_catalog.image_url, '/(\d+)\.png$'))[1]::integer = pn.dex;
