...
const TOPICS = [
  ...
  { emoji: 'x', title: 'Times Tables',  desc: '1 to 12 - pick your table' },
  { emoji: '/', title: 'Division',      desc: 'Whole-number division drills' },
  { emoji: '1/2', title: 'Fractions',   desc: 'Adding fractions, step by step' },
...
const WHY_KIDS_LOVE = [
  ...
  { emoji: '🤖', title: 'Smart AI Opponents', desc: 'No one online? AI bots challenge you at easy, medium or hard level.' },
  ...
  { emoji: '🌍', title: 'Level Brackets', desc: 'Compete against players closer to your level so matches feel fairer.' },
]
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-950 via-purple-950 to-pink-950 flex flex-col">
      ...
      <section className="border-y border-white/10 bg-white/5 py-4 px-4">
        <div className="max-w-3xl mx-auto flex flex-wrap justify-center gap-x-8 gap-y-2 text-sm text-purple-200 font-semibold">
          {['🆓 Always free', '📱 Works on phone & tablet', '🧠 Curriculum-aligned', '🔒 Safe for kids', '🌍 Play from anywhere'].map(item => (
      ...
      <section className="px-4 pb-20">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {TOPICS.map(topic => (
              <div key={topic.title} className="bg-white/5 border border-white/10 rounded-2xl p-5 text-center hover:bg-white/10 transition">
                <div className="text-4xl mb-2 font-black text-white">{topic.emoji}</div>
                <h3 className="text-white font-bold mb-1">{topic.title}</h3>
                <p className="text-purple-300 text-xs leading-relaxed">{topic.desc}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
            {HOW_IT_WORKS.map(item => (
              <div key={item.step} className="text-center">
                ...
                <div className="inline-block bg-purple-500/20 text-purple-300 text-xs font-black px-2 py-0.5 rounded-full mb-2 tracking-widest">STEP {item.step}</div>
                <h3 className="text-white font-bold text-lg mb-2">{item.title}</h3>
                <p className="text-purple-300 text-sm leading-relaxed">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="px-4 py-20">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {WHY_KIDS_LOVE.map(item => (
              <div key={item.title} className="flex gap-4 items-start bg-white/5 border border-white/10 rounded-2xl p-5 hover:bg-white/10 transition">
        ...
        <div className="max-w-2xl mx-auto text-center">
          ...
          <ul className="text-purple-300 text-sm space-y-2 text-left max-w-md mx-auto mb-8 list-none">
            {[
              ...
              '✅ Progress tracked per topic on every child profile',
